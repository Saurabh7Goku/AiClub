...
import { subscribeToProjectCanvas, updateProjectCanvas } from '@/lib/firebase/firestore';
import { CanvasPath, ProjectCanvas } from '@/types';
...
export default function CollaborativeBoard({ projectId }: CollaborativeBoardProps) {
    ...
    useEffect(() => {
        const unsub = subscribeToProjectCanvas(projectId, (canvas) => {
            if (canvas) {
    ...
    const stopDrawing = async () => {
        ...
        try {
            await updateProjectCanvas(projectId, updatedPaths);
        } catch (error) {
    ...
    const clearCanvas = async () => {
        setPaths([]);
        try {
            await updateProjectCanvas(projectId, []);
        } catch (error) {
    ...
    const downloadBoard = () => {
        ...
        link.download = `board-${projectId}.png`;
    ...
    return (
        <div className="flex flex-col h-full bg-background overflow-hidden relative">
            ...
            <div className="absolute top-6 left-6 flex flex-col gap-3 z-10">
                <div className="bg-[rgba(var(--card-bg),0.95)] backdrop-blur-md border border-white/10 rounded-2xl p-2 flex flex-col gap-1.5 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                ...
                <div className="bg-[rgba(var(--card-bg),0.95)] backdrop-blur-md border border-white/10 rounded-2xl p-2 flex flex-col gap-3 shadow-[0_0_20_rgba(0,0,0,0.5)] items-center">
            ...
            <div className="absolute top-6 right-6 flex gap-3 z-10">
                <button
                    onClick={clearCanvas}
                    className="bg-[rgba(var(--card-bg),0.8)] backdrop-blur-md border border-white/10 rounded-xl p-3 hover:bg-white/10 transition-all shadow-[0_0_15px_rgba(0,0,0,0.5)] active:scale-95 text-gray-400 hover:text-white"
                >
            ...
            {/* Status Footer */}
            <div className="bg-[rgba(var(--card-bg),0.9)] backdrop-blur-md border-t border-white/10 px-6 py-3 flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 absolute bottom-0 left-0 right-0 z-10">
                <div className="flex items-center gap-2">
                    <CursorArrowRaysIcon className="w-3.5 h-3.5 text-accent-500" />
                    Board Live: {projectId}
                </div>
                <div className="flex items-center gap-2">
                    <SwatchIcon className="w-3.5 h-3.5" style={{ color: color === '#ffffff' ? '#9ca3af' : color }} />
                    {tool} mode | Cloud Sync
                </div>
            </div>
        </div>
    );
